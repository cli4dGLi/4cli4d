...
def _finish(run: Dict[str, Any]) -> None:
    ...
    styles.child_card(f"You got {run['score']} out of 5. Lovely reading! 🌟")
...
def _feedback(run: Dict[str, Any]) -> None:
    ...
    gamify.helper_tip("Book Buddy says every try grows your reading power.", "📖")
    if st.button("Next"):
        run["idx"] += 1
        run["feedback"] = None
        run["selected_words"] = []
...
def render() -> None:
    ...
    gamify.adventure_header("Book Quest", "📖", "Help Book Buddy unlock words and stories.")
    ...
    for row_start in range(0, len(SUBTOPICS), 2):
        ...
        for offset, topic in enumerate(SUBTOPICS[row_start : row_start + 2]):
            with cols[offset]:
                if st.button(topic, key=f"english-topic-{topic}"):
                    if topic == "Read to Me":
